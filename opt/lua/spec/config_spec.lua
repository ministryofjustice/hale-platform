...
describe("config DEFAULTS", function()
    it("has required GCRA fields", function()
        ...
        assert.is_number(config_mod.DEFAULTS.penalty_ttl)
    ...
    it("has mode defaulting to 'monitor'", function()
        assert.is_string(config_mod.DEFAULTS.mode)
        assert.equals("monitor", config_mod.DEFAULTS.mode)
    end)
    ...
    it("penalty_ttl is non-negative", function()
        assert.is_true(config_mod.DEFAULTS.penalty_ttl >= 0)
    end)
end)
...
describe("config VALID_MODES", function()
    it("accepts enforce, monitor, off", function()
        assert.is_truthy(config_mod.VALID_MODES["enforce"])
        assert.is_truthy(config_mod.VALID_MODES["monitor"])
        assert.is_truthy(config_mod.VALID_MODES["off"])
    end)

    it("does not accept unknown keys", function()
        assert.is_falsy(config_mod.VALID_MODES["turbo"])
        assert.is_falsy(config_mod.VALID_MODES[""])
    end)
end)
...
describe("parse_config", function()
    ...
    it("passes mode='enforce' through without warning", function()
        local cfg, warns = config_mod.parse_config({ emission_interval=1000, burst=1000, mode="enforce" })
        assert.equals("enforce", cfg.mode)
        assert.equals(0, #warns)
    end)

    it("passes mode='monitor' through without warning", function()
        local cfg, warns = config_mod.parse_config({ emission_interval=1000, burst=1000, mode="monitor" })
        assert.equals("monitor", cfg.mode)
        assert.equals(0, #warns)
    end)

    it("passes mode='off' through without warning", function()
        local cfg, warns = config_mod.parse_config({ emission_interval=1000, burst=1000, mode="off" })
        assert.equals("off", cfg.mode)
        assert.equals(0, #warns)
    end)

    it("warns and falls back to default on unknown mode", function()
        local cfg, warns = config_mod.parse_config({ emission_interval=1000, burst=1000, mode="turbo" })
        assert.equals(config_mod.DEFAULTS.mode, cfg.mode)
        assert.equals(1, #warns)
        assert.is_truthy(warns[1]:find("mode"))
    end)

    it("partial merge: only mode key produces zero warnings", function()
        local cfg, warns = config_mod.parse_config({ mode="enforce" })
        assert.equals("enforce", cfg.mode)
        assert.equals(0, #warns)
    end)

    it("returns default penalty_ttl when absent", function()
        local cfg, warns = config_mod.parse_config({ emission_interval=1000, burst=1000 })
        assert.equals(config_mod.DEFAULTS.penalty_ttl, cfg.penalty_ttl)
        assert.equals(0, #warns)
    end)

    it("passes valid penalty_ttl through", function()
        local cfg, warns = config_mod.parse_config({ emission_interval=1000, burst=1000, penalty_ttl=600000 })
        assert.equals(600000, cfg.penalty_ttl)
        assert.equals(0, #warns)
    end)

    it("allows penalty_ttl = 0 (disables penalty key)", function()
        local cfg, warns = config_mod.parse_config({ emission_interval=1000, burst=1000, penalty_ttl=0 })
        assert.equals(0, cfg.penalty_ttl)
        assert.equals(0, #warns)
    end)

    it("floors decimal penalty_ttl", function()
        local cfg, warns = config_mod.parse_config({ emission_interval=1000, burst=1000, penalty_ttl=600000.9 })
        assert.equals(600000, cfg.penalty_ttl)
        assert.equals(0, #warns)
    end)

    it("warns and defaults when penalty_ttl is negative", function()
        local cfg, warns = config_mod.parse_config({ emission_interval=1000, burst=1000, penalty_ttl=-1 })
        assert.equals(config_mod.DEFAULTS.penalty_ttl, cfg.penalty_ttl)
        assert.equals(1, #warns)
    ...
    it("warns and defaults when penalty_ttl is non-numeric", function()
        local cfg, warns = config_mod.parse_config({ emission_interval=1000, burst=1000, penalty_ttl="bad" })
        assert.equals(config_mod.DEFAULTS.penalty_ttl, cfg.penalty_ttl)
        assert.equals(1, #warns)
    end)
