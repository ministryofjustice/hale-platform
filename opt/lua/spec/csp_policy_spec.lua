...
describe("policy.is_logged_in", function()
    it("detects the WordPress login cookie, case-insensitively", function()
        assert.is_true(policy.is_logged_in("wordpress_logged_in_abc123=hash"))
        assert.is_true(policy.is_logged_in("foo=1; WordPress_Logged_In_ABC=1; bar=2"))
        assert.is_false(policy.is_logged_in("foo=bar; comment_author=x"))
        assert.is_false(policy.is_logged_in(""))
        assert.is_false(policy.is_logged_in(nil))
    end)
end)
